-- Migration: Complete Social Features (Comments, Ratings, Favorites, Follows, Notifications) - FINAL FIX
-- Description: Adds all necessary tables and functions for a full social fitness platform
-- This version properly handles existing views and constraints

-- ============================================
-- 0. DROP DEPENDENT VIEWS FIRST
-- ============================================
DROP VIEW IF EXISTS popular_workouts CASCADE;
...
DROP POLICY IF EXISTS "Anyone can view comments on public workouts" ON workout_comments;
...
DROP POLICY IF EXISTS "Users can create comments" ON workout_comments;
CREATE POLICY "Users can create comments"
  ON workout_comments FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own comments" ON workout_comments;
CREATE POLICY "Users can update own comments"
  ON workout_comments FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own comments" ON workout_comments;
...
DROP POLICY IF EXISTS "Anyone can view ratings on public workouts" ON workout_ratings;
...
DROP POLICY IF EXISTS "Users can create ratings" ON workout_ratings;
CREATE POLICY "Users can create ratings"
  ON workout_ratings FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own ratings" ON workout_ratings;
CREATE POLICY "Users can update own ratings"
  ON workout_ratings FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own ratings" ON workout_ratings;
...
DROP POLICY IF EXISTS "Users can create favorites" ON workout_favorites;
CREATE POLICY "Users can create favorites"
  ON workout_favorites FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own favorites" ON workout_favorites;
...
DROP POLICY IF EXISTS "Anyone can record workout views" ON workout_views;
CREATE POLICY "Anyone can record workout views"
  ON workout_views FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Anyone can view workout views" ON workout_views;
...
-- Add columns to custom_workouts
DO $$
BEGIN
  -- Add average_rating
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'custom_workouts' AND column_name = 'average_rating'
  ) THEN
    ALTER TABLE custom_workouts ADD COLUMN average_rating DECIMAL(3,2) DEFAULT 0;
  END IF;

  -- Add rating_count
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'custom_workouts' AND column_name = 'rating_count'
  ) THEN
    ALTER TABLE custom_workouts ADD COLUMN rating_count INTEGER DEFAULT 0;
  END IF;

  -- Add favorite_count
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'custom_workouts' AND column_name = 'favorite_count'
  ) THEN
    ALTER TABLE custom_workouts ADD COLUMN favorite_count INTEGER DEFAULT 0;
  END IF;

  -- Add view_count
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'custom_workouts' AND column_name = 'view_count'
  ) THEN
    ALTER TABLE custom_workouts ADD COLUMN view_count INTEGER DEFAULT 0;
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'profiles' AND column_name = 'workout_count'
  ) THEN
    ALTER TABLE profiles ADD COLUMN workout_count INTEGER DEFAULT 0;
  END IF;
END $$;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE workout_comments SET like_count = like_count + 1 WHERE id = NEW.comment_id;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE workout_comments SET like_count = GREATEST(0, like_count - 1) WHERE id = OLD.comment_id;
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.parent_comment_id IS NOT NULL THEN
    UPDATE workout_comments SET reply_count = reply_count + 1 WHERE id = NEW.parent_comment_id;
  ELSIF TG_OP = 'DELETE' AND OLD.parent_comment_id IS NOT NULL THEN
    UPDATE workout_comments SET reply_count = GREATEST(0, reply_count - 1) WHERE id = OLD.parent_comment_id;
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  SET
    average_rating = (SELECT COALESCE(AVG(rating), 0) FROM workout_ratings WHERE custom_workout_id = workout_id),
    rating_count = (SELECT COUNT(*) FROM workout_ratings WHERE custom_workout_id = workout_id)
  WHERE id = workout_id;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE custom_workouts SET favorite_count = favorite_count + 1 WHERE id = NEW.custom_workout_id;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE custom_workouts SET favorite_count = GREATEST(0, favorite_count - 1) WHERE id = OLD.custom_workout_id;
  END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TRIGGER AS $$
BEGIN
  UPDATE custom_workouts SET view_count = view_count + 1 WHERE id = NEW.custom_workout_id;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  PERFORM create_notification(
    ...
    '/profile/' || NEW.follower_id, NEW.follower_id, NULL, NULL
  );
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
  ...
  workout_name TEXT;
BEGIN
  SELECT user_id, name INTO workout_owner, workout_name FROM custom_workouts WHERE id = NEW.custom_workout_id;
  IF NEW.user_id = workout_owner THEN RETURN NULL; END IF;
  SELECT display_name INTO liker_name FROM profiles WHERE id = NEW.user_id;
  PERFORM create_notification(
    workout_owner, 'like', 'Workout Liked!',
    liker_name || ' liked your workout "' || workout_name || '"',
    '/community/workout/' || NEW.custom_workout_id, NEW.user_id, NEW.custom_workout_id, NULL
  );
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  PERFORM create_notification(
    workout_owner, 'comment', 'New Comment!',
    commenter_name || ' commented on "' || workout_name || '"',
    '/community/workout/' || NEW.custom_workout_id, NEW.user_id, NEW.custom_workout_id, NEW.id
  );
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
-- ============================================
-- 11. RECREATE VIEWS (AFTER COLUMNS ARE ADDED)
-- ============================================
...
SELECT
  ...
  (COALESCE(cw.like_count, 0) * 2 + COALESCE(cw.use_count, 0) * 3 + COALESCE(cw.view_count, 0) * 1 + COALESCE(cw.favorite_count, 0) * 4) as popularity_score
