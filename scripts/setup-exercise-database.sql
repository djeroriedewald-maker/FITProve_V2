...
-- Create exercise search function
...
-- Create function to increment exercise popularity
...
-- Public read access for approved exercises
...
-- Public read access for exercise categories
CREATE POLICY "Public read access for exercise categories" ON exercise_categories
    FOR SELECT USING (true);

-- Authenticated users can create exercises (pending approval)
...
-- Admin users can do everything (you'll need to create an admin role)
-- CREATE POLICY "Admin full access" ON exercises USING (auth.jwt() ->> 'role' = 'admin');
